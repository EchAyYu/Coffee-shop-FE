...
export default function BookingFormModal({ isOpen, onClose, table, prefill }) {
  ...
  // ✅ Reset & load khi mở modal + merge dữ liệu prefill từ chatbot
  useEffect(() => {
    if (!isOpen) return;
    ...
    // Prefill trước (nếu có)
    setFormData({
      ho_ten: prefill?.ho_ten || "",
      sdt: prefill?.sdt || "",
      ngay_dat: prefill?.ngay_dat || today, // nếu không có thì mặc định hôm nay
      gio_dat: prefill?.gio_dat || "",
      so_nguoi: prefill?.so_nguoi || table?.suc_chua || 2,
      ghi_chu: prefill?.ghi_chu || "",
      id_ban: table?.id_ban || null,
    });
    ...
    // Lấy thông tin khách hàng (ghi đè tên/SĐT nếu có tài khoản)
    customers
      .getMyInfo()
      .then((res) => {
        ...
        setFormData((prev) => ({
          ...prev,
          ho_ten: info.ho_ten || info.ten_kh || prev.ho_ten || "",
          sdt: info.sdt || prev.sdt || "",
        }));
      })
      .catch(() => {
        // Khách chưa đăng nhập hoặc lỗi -> cho phép sửa
    ...
    loadMenu();
  }, [isOpen, table, prefill]); // ✅ thêm prefill vào dependency
  ...
  const handleAddToCart = (product) => {
    setCart((prevCart) => {
      const existingItem = prevCart.find(
        (item) => item.id_mon === product.id_mon
      );
      if (existingItem) {
        ...
        return [
          ...prevCart,
          { ...product, id_mon: product.id_mon, so_luong: 1 },
        ];
      }
    });
  };
  ...
  const handleUpdateQuantity = (id_mon, newQuantity) => {
    if (newQuantity <= 0) {
      setCart((prevCart) =>
        prevCart.filter((item) => item.id_mon !== id_mon)
      );
    } else {
      setCart((prevCart) =>
        prevCart.map((item) =>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={handleChange}
                      className="block w-full rounded-xl border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm p-3"
                      placeholder="Ví dụ: Cần ghế trẻ em, tổ chức sinh nhật..."
                    />
                  </div>
                          ...
                          {isLoadingMenu ? (
                            ...
                            <div className="grid grid-cols-1 gap-3">
                              {menu.map((product) => (
                                <div
                                  key={product.id_mon}
                                  className="flex items-center justify-between p-3 bg-white dark:bg-gray-700 rounded-lg shadow-sm border border-gray-100 dark:border-gray-600 hover:border-orange-300 transition-colors"
                                >
                                  ...
                                  <button
                                    ...
                                    className="p-1.5 rounded-full bg-orange-100 dark:bg-orange-900/50 text-orange-600 hover:bg-orange-200 dark:hover:bg-orange-800 transition-colors"
                                  >
                                ...
                                >
                                  ...
                                  <div className="flex items-center gap-2 bg-gray-100 dark:bg-gray-800 rounded-lg p-1">
                                    <button
                                      ...
                                      className="w-5 h-5 flex items-center justify-center rounded text-gray-600 hover:text-red-600"
                                    >
                                      {item.so_luong === 1 ? (
                                    ...
                                    <span className="font-bold text-xs w-4 text-center text-gray-800 dark:text-white">
                                      {item.so_luong}
                                    </span>
                                    <button
                                      type="button"
                                      ...
                                      className="w-5 h-5 flex items-center justify-center rounded text-gray-600 hover:text-green-600"
                                    >
